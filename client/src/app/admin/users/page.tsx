...
import { Search, ShieldAlert, GraduationCap, ArrowUpRight, BookOpen, ChevronDown } from 'lucide-react';
import { Input } from '@/components/ui/input';
import { toast } from 'sonner';

type Role = 'ALL' | 'STUDENT' | 'FACULTY' | 'ADMIN';
...
export default function RoleManagerPage() {
  ...
  const roleBadge = (role: string) => {
    if (role === 'ADMIN') return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"><ShieldAlert className="w-3 h-3" /> ADMIN</span>;
    if (role === 'FACULTY') return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-indigo-500/10 text-indigo-400 border border-indigo-500/20"><BookOpen className="w-3 h-3" /> FACULTY</span>;
    return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-slate-500/10 text-slate-400 border border-slate-500/20"><GraduationCap className="w-3 h-3" /> STUDENT</span>;
  };
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all border ${
                roleFilter === tab.value
                  ...
                  : 'bg-black/20 text-white/40 border-white/5 hover:text-white/70 hover:border-white/10'
              }`}
            >
              {tab.label}
                  ...
                  <tr key={user.id} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors">
                    <td className="py-4 px-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-500/30 to-emerald-500/30 flex items-center justify-center text-xs font-bold text-white shrink-0">
                      ...
                      {user.role !== 'ADMIN' ? (
                        <div className="flex flex-col gap-1 items-end">
                          <button
                            ...
                            className="text-xs font-bold text-emerald-400 hover:text-emerald-300 transition-colors flex items-center justify-end gap-1"
                          >
                          ...
                          {user.role !== 'FACULTY' && (
                            <button
                              ...
                              className="text-xs font-bold text-indigo-400 hover:text-indigo-300 transition-colors flex items-center justify-end gap-1"
                            >
                          ...
                          {user.role !== 'STUDENT' && (
                            <button
                              onClick={() => grantRole(user.id, user.role, 'STUDENT')}
                              className="text-xs font-bold text-slate-400 hover:text-slate-300 transition-colors flex items-center justify-end gap-1"
                            >
                              Demote to Student <ChevronDown className="w-3 h-3" />
                            </button>
                          )}
                        </div>
                      ) : (
